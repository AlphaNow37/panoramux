use crate::theory::datastrutures::graph::LinkGraph;
use crate::theory::math::polygon::are_counter_clockwise;
...
impl Triangulation {
    ...
    pub fn localise(&self, pos: Vec2) -> Option<usize> {
        'a: for tri_i in 0..self.triangles.len() {
            for tae in self.triangles[tri_i] {
                ...
                let prod = (pos-pos1).dot((pos2-pos1).rotate_ccw());
                if prod < 0. {
                    continue 'a;
                }
            }
    ...
    pub fn to_vertex_graph(&self) -> LinkGraph {
        let mut graph = LinkGraph::new(self.vertex_poss.len());
        for tri in &self.triangles {
            for edge in tri {
                graph.push_double_link(edge.verts[0], edge.verts[1]);
            }
        }
        graph
    }
    pub fn to_triangle_graph(&self) -> LinkGraph {
        let mut graph = LinkGraph::new(self.triangles.len());
    ...
    pub fn get_tri_center(&self, i: usize) -> Vec2 {
