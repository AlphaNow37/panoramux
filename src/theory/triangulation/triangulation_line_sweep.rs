...
use crate::theory::datastrutures::skip_list::{Cursor, Interval, IntervalSkipLists, SkipListAccess};
use crate::theory::math::polygon::{are_counter_clockwise, Polygon};
use crate::theory::math::Vec2;
use crate::theory::triangulation::Triangulation;
use crate::theory::triangulation::triangulation::TriAdjacentEdge;
...
enum VertexKind {
    WallTop(Vec2),
    WallBot(Vec2),
    Source([Vec2; 2]), // top-down
    Sink,
    Merge,
    Split([Vec2; 2]), // top-down
}
...
enum AreaCover {
    ...
    Interval(f32, f32), // down-top
}

impl AreaCover {
    /// p1, p2 are counterclockwise from the point of vue of the empty space
    fn from_poss(p1: Vec2, p2: Vec2) -> Self {
        if p1[1] >= p2[1] {
            AreaCover::Interval(p2.y(), p1.y())
        } else if p1[0] <= p2[0] {
            AreaCover::NoneTop
        } else {
            AreaCover::NoneBot
...
struct FrontInterval {
    top: [Vec2; 2], // left-right
    bot: [Vec2; 2], // left-right
...
impl Interval for FrontInterval {
    ...
    fn cmp_value(&self, value: &Self::Value) -> Ordering {
        ...
        if dir1.rotate_cw().dot(delta1) < 0. {
            return Ordering::Less;
        }
        if dir2.rotate_ccw().dot(delta2) < 0. {
...
fn propagate_up(
    cursor: &mut Cursor<AreaInterval>,
    ...
    loop {
        ...
        if (p2 - p1).rotate_ccw().dot(pos - p1) < 0. {
            cursor.move_right();
            cursor.move_right();
            return;
        }
    ...
    for i in 0..poly.len() {
        ...
        vertices.push(Vertex {
            ...
            kind: match (
                delta_prev[0] > 0.,
                delta_next[0] > 0.,
                delta_prev.dot(delta_next.rotate_ccw()) > 0.,
            ) {
...
// The enveloppe should be CW, the holes CCW
pub fn triangulate_line_sweep(enveloppe: Polygon, holes: &[Polygon]) -> Triangulation {
    let mut vertices = Vec::new();
    
    for poly in holes {
        add_polygon(&mut vertices, &poly);
    }
    add_polygon(&mut vertices, &enveloppe);
